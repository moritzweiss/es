...
class G2PPModel:
    ...
    def interest_rate(self, dt, x_start, y_start, t_start):
        ...
        return self.psi(t_start+dt) + x + y
    
    def interest_rate_path(self, n_steps, x_start, y_start, t_start, dt):
        # x,y,z at time t_start
        x_path = [x_start]
        y_path = [y_start]
        r_path = [self.interest_rate(0, x_start, y_start, t_start)]
        for step in range(1, n_steps):
            x, y = self.coupled_ou.coupled_increment(x_path[-1], y_path[-1], dt)
            r = self.interest_rate(dt, x, y, t_start + step*dt)
            x_path.append(x)
            y_path.append(y)
            r_path.append(r)
...
if __name__ == "__main__":
    ...
    r = model.interest_rate(dt=100, x_start=start_x, y_start=start_y, t_start=0)
    plt.figure()
    ...
    n_steps = 101 
    r, x, y = model.interest_rate_path(n_steps=n_steps, x_start=start_x, y_start=start_y, t_start=0, dt=1)
    plt.figure()
