...
def expected_shortfall(
    ...
    normalize = False,
    make_decreasing = False ) -> Tuple[torch.tensor, int]:
    """
...
if __name__ == "__main__":
    ...
    alpha = 0.7
    es = losses[:3].sum()/3
    out, j = expected_shortfall(losses=losses, alpha=alpha, normalize=normalization)
    ...
    es_out, j_out = expected_shortfall(losses=losses, alpha=alpha, normalize=normalization)
    ...
    alpha = 0.7
    es_w, j_w = expected_shortfall(losses=losses,alpha=alpha, sample_weights=weights, normalize=False, make_decreasing=True)
